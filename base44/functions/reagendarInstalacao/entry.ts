...
// Reagenda uma instalação no SolarFlow. Para projetos legacy com evento no
// Google Calendar, também atualiza o evento (best-effort).
export default async function(req) {
  try {
    ...
    // Shift continuações pelo mesmo delta de dias
    ...
    // Legacy: atualiza eventos no Google Calendar se existirem (best-effort)
    const hasGoogleEvents = fresh.google_calendar_event_id ||
      (Array.isArray(fresh.google_calendar_event_ids) && fresh.google_calendar_event_ids.length > 0);
    if (hasGoogleEvents) {
      try {
        ...
        console.warn('[reagendarInstalacao] Falha ao atualizar Google Calendar (legacy):', e?.message);
      }
    }
